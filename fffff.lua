...
local function getMod(name)
    if _G.DisabledModules and _G.DisabledModules[name] then
        warn("[dyumra] Module disabled by user selector:", name)
        return nil
    end
    ...
    if not path then
        warn("[dyumra] Path not found for module:", name)
        return nil
    end
    local localPath = "DYHUBTeam/dyumra_Source/" .. path
    if readfile and isfile and isfile(localPath) then
        ...
        if fn then
            ...
            if runSuccess then
                ...
                warn("[dyumra] Error executing local module '" .. tostring(name) .. "': " .. tostring(runRes))
            end
        else
            warn("[dyumra] Failed to compile local module '" .. tostring(name) .. "': " .. tostring(err))
        end
    end
    ...
    if success and res then
        ...
        if not fn then
            warn("[dyumra] Failed to load module '" .. tostring(name) .. "': " .. tostring(err))
            return nil
        end
        local runSuccess, runRes = pcall(fn)
        if not runSuccess then
            warn("[dyumra] Error executing module '" .. tostring(name) .. "': " .. tostring(runRes))
            return nil
        end
        ModuleCache[name] = runRes
        return runRes
    else
        warn("[dyumra] Failed to download module '" .. tostring(name) .. "' after 3 attempts.")
        return nil
    end
end
_G.getMod = getMod
...
pcall(function()
    ...
    elseif game and game:GetService("ReplicatedStorage"):FindFirstChild("DYHUB_Core") then
        Speed_Library = require(game:GetService("ReplicatedStorage"):FindFirstChild("DYHUB_Core"))
    elseif game and game:GetService("ReplicatedStorage"):FindFirstChild("dyumra_Core") then
        Speed_Library = require(game:GetService("ReplicatedStorage"):FindFirstChild("dyumra_Core"))
    elseif readfile and isfile and isfile("DYHUBTeam/GUIENC.lua") then
        Speed_Library = loadstring(readfile("DYHUBTeam/GUIENC.lua"))()
    elseif readfile and isfile and isfile("GUIENC.lua") then
        Speed_Library = loadstring(readfile("GUIENC.lua"))()
    elseif readfile and isfile and isfile("dyumra_Source/DYHUB_UILib.lua") then
        Speed_Library = loadstring(readfile("dyumra_Source/DYHUB_UILib.lua"))()
...
if not Speed_Library then
    warn("[dyumra] Gagal load GUIENC!")
    return
end
_G.Speed_Library = Speed_Library
task.spawn(function()
    pcall(function()
        local ideScript = game:HttpGet("https://raw.githubusercontent.com/KAN-FISCH/FischTes/refs/heads/main/DYHUBIDE")
    ...
    local function saveSavedKey(Key)
        if writefile then
            pcall(function()
                writefile("DYHUBKey.txt", tostring(Key))
            end)
        end
    end
    local function getSavedKey()
        if isfile and isfile("DYHUBKey.txt") and readfile then
            local ok, content = pcall(readfile, "DYHUBKey.txt")
            if ok then
    ...
    local function createPremiumKeyUI(Info, Exclusive, AutosTab, AreaTab, EspTab, Misc, SettingsTab, Speed_Library)
        ...
        local userKey = "DYHUB_3gd61bd8nak"
        if genvKey ~= "" then
            userKey = genvKey
        ...
        if userKey ~= "DYHUB_3gd61bd8nak" then
            _G.Key = userKey
            getgenv().Key = userKey
        end
        ...
        if not (Info and Info.ScrolLayers) then
            local KeySection = Info:AddSection("Premium Information")
            local StatusPara = KeySection:AddParagraph({
                Title = "Key Status: Checking...",
                Content = "Status: Limited key..."
            })
            local function setStatusPara(title, content)
                pcall(function()
                    if not StatusPara then return end
                    if StatusPara.SetTitle and StatusPara.SetContent then
                        StatusPara:SetTitle(title)
                        StatusPara:SetContent(content)
                    elseif StatusPara.Set then
                        StatusPara:Set({ Title = title, Content = content })
                    end
                end)
            end
            local function doValidate(keyToTest, isAuto)
                if not keyToTest or keyToTest == "" then
                    setStatusPara("Key Status: Premium User", "Status: Premium User\nMasukkan key premium untuk membuka semua fitur.")
                    pcall(function()
                        if Exclusive and Exclusive.Unlock then
                            Exclusive:Unlock()
                        end

                        if AutosTab and AutosTab.Unlock then
                            AutosTab:Unlock()
                        end

                        if EspTab and EspTab.Unlock then
                            EspTab:Unlock()
                        end

                        if Misc and Misc.Unlock then
                            Misc:Unlock()
                        end

                        if SettingsTab and SettingsTab.Unlock then
                            SettingsTab:Unlock()
                        end
                    end)
                ...
                task.spawn(function()
                    ...
                    if isValid then
                        ...
                        setStatusPara("Key Status: Lifetime (Premium)", "Status: Premium User\nPaid: " .. tostring(sisaWaktu) .. "\nExpired: " .. tostring(waktuExpired))
                    else
                        setStatusPara("Key Status: TIDAK VALID", "Status: Free User\nAlasan: " .. tostring(msg or "Key salah / expired"))
                    end
                end)
            end
            if userKey ~= "" then
                ...
                setStatusPara("Key Status: Free User", "Masukkan key premium Anda di atas lalu klik Validasi Key.")
            end
            return
        end
        ...
        local DYHUBIcon = Create("ImageLabel", {
        ...
        Create("TextLabel", {
            Text = "Jadi bagian dari komunitas premium DYHUBTeam!",
        ...
        getKeyBtn.Activated:Connect(function()
            local link = "https://key.DYHUBteam.asia/"
        ...
        Create("TextLabel", {
            Text = "Tips: Dapatkan key premium hanya di server resmi DYHUBTeam untuk keamanan akun Anda.",
            Font = Enum.Font.Gotham,
            TextColor3 = Color3.fromRGB(140, 140, 150),
            TextSize = 8,
            Position = UDim2.new(0, 20, 0, 0),
                ...
                pcall(function()
                    for _, desc in ipairs(container:GetDescendants()) do
                        if desc:IsA("TextLabel") or desc:IsA("TextButton") then
                            ...
                            if type(txt) == "string" and txt:find("DYHUBTeam") and txt:find("Executor") then
            ...
            pcall(function()
                for _, desc in ipairs(game:GetDescendants()) do
                    if (desc:IsA("TextLabel") or desc:IsA("TextButton")) then
                        ...
                        if ok and type(txt) == "string" and txt:find("DYHUBTeam") and txt:find("Executor") then
...
local function startSpearFarmLoop()
    ...
    task.spawn(function()
        ...
        while _G.Config.EnableSpearCatch do
            ...
            if not ok then
                warn("[dyumra] Error in Spear Farm: " .. tostring(err))
                task.wait(1)
            end
        end
    end)
...
local function setupGUI()
    local Window = Speed_Library:CreateWindow({
        Title = "DYHUB x Shield HUB | Executor : " .. executorName,
        Description = "Fisch • Kaitun Version",
        ["Tab Width"] = 110,
    ...
    local Info        = GrpMain:CreateTab({ "Info",     "", "Information & Event" })
    ...
    local SettingsTab = GrpSettings:CreateTab({ "Setting", "", "Config & Settings" })
    ...
    local function loadPrivateServers()
        local success, res = pcall(function()
            return game:HttpGet("https://key.DYHUBteam.asia/api/private-servers")
        end)
        if success and res then
            ...
            if not decodeSuccess then
                warn("[dyumra] JSON Decode Error: " .. tostring(servers))
                warn("[dyumra] Response: " .. tostring(res):sub(1, 300))
            end
    ...
    ExclusiveSection:AddButton({
        ...
        Callback = function()
            local HttpService = game:GetService("HttpService")
            pcall(function()
                writefile("DYHUBTeamConfig.json", HttpService:JSONEncode(_G.Config))
            end)
        end
    })
    ...
    local CreditsSection = Infr
    if CreditsSection then
        CreditsSection:AddParagraph({
            Title = "DYHUB | dyumra",
            Content = "Full GUI Layout Re-added.\nLibary by Speed, Thank for Support US from dyumra!!"
        })
        CreditsSection:AddButton({
            Title = "Open DYHUB Armoury Crate",
            Description = "Spin for game we are supported!",
            Callback = function()
                if Window and (Window.OpenDYHUBLauncher or Window.OpenROGLauncher) then
                    local openFn = Window.OpenDYHUBLauncher or Window.OpenROGLauncher
                    openFn(Window)
                elseif Speed_Library and (Speed_Library.CreateDYHUBLauncher or Speed_Library.CreateROGLauncher) then
                    local createFn = Speed_Library.CreateDYHUBLauncher or Speed_Library.CreateROGLauncher
                    createFn(Speed_Library, {
                        Title = "DYHUB Armoury Crate",
                        AutoDetect = true,
                        OnLaunch = function(selectedGame)
                            if Window and Window.SetVisible then Window:SetVisible(true) end
                        end,
                        OnClose = function()
                            if Window and Window.SetVisible then Window:SetVisible(true) end
                        end
    ...
    getgenv().NewFishWindow = Window
    _G.NewFishWindow = Window
end
local guiOk, guiErr = pcall(setupGUI)
if not guiOk then
    warn("[dyumra] Error setupGUI: " .. tostring(guiErr))
else
    print("[dyumra] GUI Loaded Successfully from ReplicatedStorage!")
end
task.spawn(function()
    task.wait(1)
    if _G.Config then
